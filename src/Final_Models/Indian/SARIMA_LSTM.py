...
for bin_id in bins:
    ...
    sarima_fitted_train = sarima_fit.fittedvalues
    residuals = train - sarima_fitted_train

    # --- LSTM on Residuals ---
    scaler = MinMaxScaler()
    scaled_residuals = scaler.fit_transform(residuals.dropna().values.reshape(-1, 1))
    ...
    # Generate LSTM predictions on SARIMA forecast horizon
    ...
    # --- Hybrid Forecast = SARIMA + Residual LSTM ---
    ...
    results_hybrid[bin_id] = {"RMSE": rmse, "MAE": mae, "MAPE": mape}
    ...
    plt.plot(
        test.index,
        hybrid_forecast,
        label="SARIMA + LSTM Predicted Fullness",
        color="red",
        linewidth=2.5,
        marker="o",
        markersize=6,
    ...
    plt.title(f"Hybrid SARIMA + LSTM - Bin {bin_id}", fontsize=12)
    ...
    print(
        f"[Hybrid] Bin {bin_id} → RMSE: {rmse:.2f}, MAE: {mae:.2f}, MAPE: {mape:.2f}%"
    )
